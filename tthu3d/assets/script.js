...
function getRandomWish(currentText = "") {
  const pool = wishList.filter((item) => item.text !== currentText);
  const source = pool.length > 0 ? pool : wishList;
  const selected = source[Math.floor(Math.random() * source.length)];

  return {
    text: selected.text,
    img: selected.img || getRandomImage(),
  };
}
...
function onPointerUp(event) {
  ...
  if (intersects.length > 0) {
    ...
    const randomWish = getRandomWish(selectedLantern.userData.wish);
    selectedLantern.userData.wish = randomWish.text;
    selectedLantern.userData.imgUrl = randomWish.img;

    wishText.textContent = `"${randomWish.text}"`;
    wishImage.src = randomWish.img;
    wishImage.alt = "Hình ảnh Trung Thu";
